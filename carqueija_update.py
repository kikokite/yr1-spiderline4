...
def display_menu():
    ...
    title = font.render("SpiderLine4", True, BLACK)
    ...
    while not board.game_over:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if mode == "pvp" or (mode == "pvc" and board.turn == 0):
                    mouse_x, mouse_y = pygame.mouse.get_pos()
                    col = (mouse_x - (WINDOW_WIDTH - board.col_count * CELL_SIZE) // 2) // CELL_SIZE
                    row = (mouse_y - (WINDOW_HEIGHT - board.row_count * CELL_SIZE) // 2) // CELL_SIZE
                ...
                elif mode == "pvc" and board.turn == 1:
                    # Random move for CPU player
                    action = random.choice(board.actions())
                    board.put_piece(board.turn + 1, action[0], action[1])
